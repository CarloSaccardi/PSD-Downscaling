...
class SimMIMSwinUNETR(SwinUNETR):
    ...
    def forward(self, x_in, patch_mask):
        ...
        if not self.use_light_decoder:
            
            # --- 2. Create pixel-masked input for CONV skips ---
            x_in_pixel_masked = x_in * pixel_mask

            # --- 3. Feed pixel-masked input to CONV skips (enc0 and enc1) ---
            # LEAK 1 PLUGGED
            enc0 = self.encoder1(x_in_pixel_masked) 

            # LEAK 2 PLUGGED
            x0_pixel_masked = self.swinViT.patch_embed(x_in_pixel_masked)
            x0_pixel_masked_out = self.swinViT.proj_out(x0_pixel_masked, self.normalize)
            enc1 = self.encoder2(x0_pixel_masked_out) 
        
        # --- 4. Create TOKEN-masked input for TRANSFORMER backbone ---
        ...
        # --- 5. Reshape token-masked input for SwinViT layers ---
        x_token_masked = x_token_masked_flat.transpose(1, 2).view(B, C, x_embed_shape[2], x_embed_shape[3])

        # --- 6. Run TRANSFORMER backbone on token-masked input ---
        x = x_token_masked
        ...
        # Note: We do not need x0_out, as we already handled enc1
        hidden_states_out = [None, x1_out, x2_out, x3_out, x4_out] 

        # --- 7. Run the decoder ---
        if self.use_light_decoder:
            # 7a. Run the "light decoder"
            # We still use encoder10 as the bottleneck block
            dec4 = self.encoder10(hidden_states_out[4])
            logits = self.light_decoder_head(dec4)
            
        else:
            # 7b. Run the "heavy" U-Net decoder (original logic)
