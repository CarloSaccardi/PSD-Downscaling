...
class GeoUNet(Module):
    ...
    def forward(self, x, features, noise_labels, class_labels, augment_labels=None):
        with nvtx.annotate(
            ...
            aux = None
            tmp = None
            feat_dict = {f.shape[-1]: f for f in features} if features is not None else {}
                ...
                with nvtx.annotate(
                    ...
                    if "aux_up" in name:
                        ...
                        if features is not None:
                            # [NEW] C. Hierarchical Cross-Attention Injection
                            # 1. Get current spatial resolution (e.g., 24)
                            res = x.shape[-1]
                            # 2. Check if we have a registered attention layer for this resolution
                            attn_key = f"dec_attn_{res}x{res}"
                            
                            # 3. Apply conditioning if the attention block and Swin feature exist
                            if attn_key in self.cross_attn and res in feat_dict:
                                # We "interrupt" the flow here to inject global context.
                                # x (local features) queries feat_dict[res] (global Swin map)
                                x = self.cross_attn[attn_key](x, feat_dict[res])
            return aux


class GeoUNetPosEmbd(GeoUNet):
...
class CrossAttentionBlock(torch.nn.Module):
    def __init__(self, query_dim, context_dim, num_heads=4):
        ...
        self.to_q = Linear(query_dim, query_dim, bias=False)
        self.to_k = Linear(context_dim, query_dim, bias=False)
        self.to_v = Linear(context_dim, query_dim, bias=False)
        self.to_out = Linear(query_dim, query_dim)

    def forward(self, x, context):
        # x: [B, C, H, W] (UNet feature)
        # context: [B, C_z, H_z, W_z] (Swin hierarchical feature)
        b, c, h, w = x.shape
        
        q = self.to_q(x.flatten(2).transpose(1, 2)) # [B, HW, C]
        k = self.to_k(context.flatten(2).transpose(1, 2)) # [B, H_zW_z, C]
        v = self.to_v(context.flatten(2).transpose(1, 2)) # [B, H_zW_z, C]
        ...
        # Cross-attention: Q comes from UNet (local), K/V from Swin (global)
        attn = (q @ k.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        
        out = (attn @ v).transpose(1, 2).reshape(b, h, w, c)
        return x + self.to_out(out).permute(0, 3, 1, 2)
