...
from MONAI.monai.networks.nets.swin_unetr import SwinUNETR
from MONAI.monai.utils import ensure_tuple_rep
...
class SwinUNetrWrapper(pl.LightningModule):
    def __init__(self, args, upsample="vae", dim=3072):
        ...
        window_size = ensure_tuple_rep(7, args.spatial_dims)  # (7, 7) for 2D
        
        # Use the original SwinUNETR architecture for 2D
        # For reconstruction, out_channels should match in_channels
        self.swin_unetr = SwinUNETR(
            in_channels=args.in_channels,
            out_channels=args.in_channels,  # Reconstruction: output same as input
            patch_size=2,
            depths=[2, 2, 2, 2],
            num_heads=[3, 6, 12, 24],
            ...
            spatial_dims=args.spatial_dims,  # Should be 2 for 2D data
            downsample="merging",
            use_v2=False,
        )
